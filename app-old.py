...
client = genai.Client(api_key='AIzaSyDOq5uX83g0CYA86R0H1H4Fo6zNTywKqJY')


def get_rank_trend(df, group_col, current_year, lookback_years=3):
    """Calcule le changement de rang par rapport à N années en arrière."""
    start_year = max(df['Year'].min(), current_year - lookback_years)

    # Rang actuel
    current_rank = df[df['Year'] == current_year][group_col].value_counts().rank(ascending=False, method='min')

    # Rang passé
    past_rank = df[df['Year'] == start_year][group_col].value_counts().rank(ascending=False, method='min')

    trends = {}
    for item in current_rank.index:
        if item in past_rank:
            diff = past_rank[item] - current_rank[item]  # Si past=5 et current=2, diff=+3 (gain de places)
            if diff > 0:
                trends[item] = f"▲)"
            elif diff < 0:
                trends[item] = f"▼"
...
if vue_donnee:
    ...
    st.markdown("---")
    st.markdown(header_with_info(2, "Vue des données", "Vue données", "Cette vue présente les données filtrées sous forme de tableau interactif. Vous pouvez trier, rechercher et explorer les données en détail."), unsafe_allow_html=True)
    ...
    with tab1:
        ...
        display_cols = ['DENOMINATIONPROPOSAL', 'SPECIENAME', 'CompanyGroup', 'Sector', 'PublicationType', 'Year', 'COUNTRYID']
        available_cols = [c for c in display_cols if c in df_filtered.columns]

        df_display = df_filtered[available_cols].copy()
        df_display.columns = ['Variété', 'Espèce', 'Entreprise', 'Secteur', 'Type', 'Année', 'Pays'][:len(available_cols)]
    ...
    with tab3:
        ...
        company_summary.columns = ['Entreprise', 'Nb variétés', 'Nb espèces', 'Secteurs', 'Première année', 'Dernière année']
    ...
    with tab4:
        ...
        sector_summary.columns = ['Secteur', 'Nb variétés', 'Nb espèces', 'Nb entreprises', 'Première année', 'Dernière année']
    ...
    st.markdown(header_with_info(2, "1. Evolution temporel par type de demande", "Evolution temporelle", "Ces graphiques montrent l évolution du nombre de demandes de variétés végétales au fil du temps, séparées par type: NLI (National Listing) pour les inscriptions nationales et PBR (Plant Breeders Rights) pour les droits d obtention végétale européens."), unsafe_allow_html=True)
    ...
    avg_nli = len(df_filtered[df_filtered['PublicationType'] == 'National Listing']) / (year_range[1] - year_range[0] + 1)
    avg_pbr =  len(df_filtered[df_filtered['PublicationType'] == 'Plant Breeders Rights']) / (year_range[1] - year_range[0] + 1)
    tc_nli = 0
    try:
    ...
    st.markdown(header_with_info(2, "2. Top espèces et secteurs", "Top espèces et secteurs", "Cette section présente les 15 espèces végétales les plus demandées ainsi que la répartition des demandes par secteur d activité (Agriculture, Fruits, Légumes, Ornementales). Cliquez sur une barre pour voir les détails de l espèce."), unsafe_allow_html=True)
    ...
    with col1:
        ...
        trends_specie, start_yr_specie = get_rank_trend(df_filtered, 'SpecieGroupShort', year_range[1])
        tooltip_specie = f"Les flèches indiquent l'évolution du rang de l'espèce entre {start_yr_specie} et {year_range[1]}."
        ...
        st.plotly_chart(fig_species, use_container_width=True, key="species_chart")

    with col2:
        subcol1, subcol2 = st.columns(2)

        # Calcul des tendances pour les secteurs
        trends_sect, start_yr_sect = get_rank_trend(df_filtered, 'Sector', year_range[1])
        tooltip_sect = f"Indique si le secteur a gagné ou perdu en importance dans le volume global des dépôts entre {start_yr_sect} et {year_range[1]}."
    ...
    st.markdown(header_with_info(2, "3. Top entreprises", "Top entreprises", "Ce graphique montre les 10 entreprises ayant déposé le plus de demandes de variétés végétales. Les statistiques incluent le nombre total d entreprises, la moyenne de dépôts par entreprise et la concentration du marché (part des 10 premières entreprises)."), unsafe_allow_html=True)
    ...
    with col1:
        trends_comp, start_yr_comp = get_rank_trend(df_filtered, 'CompanyGroup', year_range[1])
        tooltip_comp = f"Évolution de la position de l'entreprise dans le classement entre {start_yr_comp} et {year_range[1]}."
    ...
    if has_nli or has_pbr:
        ...
        if has_pbr:
            df_strategy_pivot['PBR_Percentage'] = (df_strategy_pivot['Plant Breeders Rights'] / df_strategy_pivot['Total'] * 100).round(2)
        else:
    ...
    st.markdown(header_with_info(3, "Evolution des proportions de variété protégées (PBR) parmis les variétés listés (NLI) pour les 10 principales entreprises", "Stratégie par entreprise", "Ce graphique compare les stratégies de protection des 10 principales entreprises au fil du temps. Chaque ligne représente le pourcentage de PBR d une entreprise, permettant d identifier les différences de stratégies de protection intellectuelle entre les acteurs du marché."), unsafe_allow_html=True)
    ...
    st.markdown(f'''<div class="title-with-info"><span style="font-size: 14px; font-weight: 500;">Sélectionnez une ou plusieurs dimensions d'analyse:</span>{info_icon("Analyse détaillée", "Cette section permet d analyser en détail l évolution des demandes NLI et PBR selon différentes dimensions. Les courbes pleines représentent les NLI, les courbes en pointillés les PBR, et les courbes en points le ratio PBR/NLI. Vous pouvez combiner plusieurs dimensions pour une analyse croisée.")}</div>''', unsafe_allow_html=True)
